(function initLucubroI18n(root) {
  ...
  const phraseEntries = [
    ['Adjust next lesson', '调整下一课', '次のレッスンを調整'],
    ...
    ['Artifact navigation', '作品导航', '成果物ナビゲーション'],
    ['Courses and materials', '课程与材料', 'コースと教材'],
    ['New artifact', '新建作品', '新しい成果物'],
    ['Start a Zhihu answer', '开始一篇知乎回答', '知乎回答を始める'],
    ['Create the task owner first. You can link an existing course now or add source material later.', '先建立任务本身。你可以现在关联已有课程，也可以稍后添加材料。', 'まずタスク本体を作成します。既存コースを今リンクするか、教材を後で追加できます。'],
    ['Zhihu question', '知乎问题', '知乎の質問'],
    ['Use the exact question the answer must address.', '填写这篇回答必须回应的原始问题。', '回答で扱う質問をそのまま入力します。'],
    ['Target reader', '目标读者', '対象読者'],
    ['Minimum usable standards', '最低可用标准', '最低限の完成基準'],
    ['The central claim is clear', '中心结论明确', '中心主張が明確'],
    ['Key claims use source evidence', '关键结论有材料证据', '重要な主張に教材の根拠がある'],
    ['The answer helps the target reader act', '回答能帮助目标读者行动', '対象読者が行動できる回答'],
    ['Body privacy', '正文隐私', '本文のプライバシー'],
    ['Save body locally', '在本地保存正文', '本文をローカル保存'],
    ['Draft and explicit checkpoints stay on this device.', '草稿与显式检查点仅保存在当前设备。', '下書きと明示的なチェックポイントはこの端末にのみ保存されます。'],
    ['Store structure only', '只保存结构化证据', '構造化情報のみ保存'],
    ['Draft and revision body text are never written to disk.', '草稿与修订正文不会写入磁盘。', '下書きと改訂本文はディスクに保存されません。'],
    ['Existing course (optional)', '已有课程（可选）', '既存コース（任意）'],
    ['Link source material later', '稍后关联材料', '教材を後でリンク'],
    ['Create artifact', '创建作品', '成果物を作成'],
    ['New artifact | Lucubro', '新建作品 | Lucubro', '新しい成果物 | Lucubro'],
    ['Create a local artifact task and optionally link course material.', '创建一个本地作品任务，并可选择关联课程材料。', 'ローカルの成果物タスクを作成し、必要に応じてコース教材をリンクします。'],
    ['Artifact | Lucubro', '作品 | Lucubro', '成果物 | Lucubro'],
    ['Review an artifact task, its standards, linked course, and current gaps.', '查看作品任务、完成标准、关联课程和当前缺口。', '成果物タスク、完成基準、リンクしたコース、現在のギャップを確認します。'],
    ['Loading artifact', '正在加载作品', '成果物を読み込み中'],
    ['Artifact unavailable', '作品不可用', '成果物を利用できません'],
    ['The artifact could not be loaded.', '无法加载这个作品。', '成果物を読み込めませんでした。'],
    ['Draft', '草稿', '下書き'],
    ['Waiting for source', '等待材料', '教材を待機中'],
    ['Waiting for mission', '等待 Mission', 'Mission を待機中'],
    ['Revising', '修订中', '改訂中'],
    ['Ready', '已就绪', '準備完了'],
    ['Delivered', '已交付', '提出済み'],
    ...
    ['Rejected', '已拒绝', '却下済み'],
    ['Modified', '已调整', '調整済み'],
    ['Orphaned', '锚点失效', 'アンカー切れ'],
    ['Resolved', '已解决', '解決済み'],
    ['Document body', '文档正文', '文書本文'],
    ['Flag-A provides a safe route and real data only. Autosave and the productized writing workspace are not enabled.', 'Flag-A 仅提供安全路由和真实数据，尚未启用自动保存或产品化写作工作区。', 'Flag-A は安全なルートと実データのみを提供します。自動保存と製品版の執筆ワークスペースはまだ有効ではありません。'],
    ['Current local draft', '当前本地草稿', '現在のローカル下書き'],
    ['No body is saved yet', '尚未保存正文', '本文はまだ保存されていません'],
    ['Body text is not stored for this artifact. Paste a temporary excerpt only when calling critique.', '此作品不保存正文。仅在请求 critique 时临时粘贴片段。', 'この成果物では本文を保存しません。critique を実行するときだけ一時的な抜粋を貼り付けます。'],
    ['Task contract', '任务合同', 'タスク契約'],
    ['Not available yet', '尚不可用', 'まだ利用できません'],
    ['Current gaps', '当前缺口', '現在のギャップ'],
    ['No critique gaps yet', '尚无 critique 缺口', 'critique のギャップはまだありません'],
    ['Open linked course', '打开关联课程', 'リンクしたコースを開く'],
    ['No standards', '暂无标准', '基準はありません'],
    ['Artifact saved, but source material was not linked:', '作品已保存，但材料尚未关联：', '成果物は保存されましたが、教材をリンクできませんでした：'],
    ['The request could not be completed.', '请求未能完成。', 'リクエストを完了できませんでした。'],
    ...
    ['How should the next lesson adjust?', '下一课该怎样调整？', '次のレッスンをどう調整しますか？'],
    ['Choose one signal. Feedback is optional and never blocks progress.', '点一下即可。反馈可选，不会阻止继续学习。', '1つ選んでください。フィードバックは任意で、学習の続行を妨げません。'],
    ['Fits my problem', '切合痛点', '今の課題に合っている'],
    ['Skip this direction', '不相关，后续跳过', '関連がないので今後は避ける'],
    ['Move faster', '更快通读', 'もっと速く進める'],
    ['Go deeper', '更深挖掘', 'もっと深く掘り下げる'],
    ['Optional detail', '补充说明（可选）', '補足（任意）'],
    ['What should Lucubro keep or change?', '希望 Lucubro 保留或调整什么？', 'Lucubro に残してほしい点、変えてほしい点は？'],
    ['Skip feedback and continue', '跳过反馈并继续', 'フィードバックを省略して続ける'],
    ['Feedback could not be saved. Continuing to the next lesson.', '反馈暂时未保存，仍会继续生成下一课。', 'フィードバックを保存できませんでしたが、次のレッスンへ進みます。'],
    ['Expected output', '期望产出', '期待する成果物'],
    ['Success evidence', '成功证据', '成功の証拠'],
    ['Problem statement', '问题陈述', '課題の説明'],
    ['Assistant', '助手', 'アシスタント'],
    ...
    ['Cancel generation', '取消生成', '生成をキャンセル'],
    ['Cancelling generation…', '正在取消生成…', '生成をキャンセルしています…'],
    ['Course generation cancelled', '课程生成已取消', 'コース生成をキャンセルしました'],
    ['The course generation was cancelled. Your material and confirmed learning settings are preserved.', '课程生成已取消。材料和已确认的学习设置仍然保留。', 'コース生成をキャンセルしました。教材と確認済みの学習設定は保持されています。'],
    ['Start again', '重新开始', 'もう一度開始'],
    ['Restarting generation…', '正在重新开始生成…', '生成を再開しています…'],
    ['Generation is no longer cancellable. The latest server state is shown.', '当前生成已不可取消，页面已按服务器最新状态更新。', '生成はすでにキャンセルできません。サーバーの最新状態を表示しています。'],
    ['The generation could not be cancelled. The current server state is still active.', '暂时无法取消生成，服务器仍显示任务进行中。', '生成をキャンセルできませんでした。サーバーではまだ実行中です。'],
    ...
    ['Your library', '你的课程库', 'コースライブラリ'],
    ['Every course you build, in one place.', '你建的每门课，都在这里。', '作成したコースはすべてここに。'],
    ['Pick up where you left off, or start a new course from your own material.', '从上次停下的地方继续，或用你自己的材料新建一门课。', '続きから学習するか、自分の教材で新しいコースを始められます。'],
    ...
    ['Scroll horizontally to review the full year. Use arrow keys to move between days.', '横向滚动可查看全年记录。使用方向键在日期之间移动。', '横にスクロールすると1年分を確認できます。矢印キーで日付間を移動できます。'],
    ['Less', '少', '少'],
    ['More', '多', '多'],
    ['Notebook', '笔记本', 'ノートブック'],
    ...
    ['Use dark reading theme', '使用深色阅读主题', 'ダーク読書テーマを使う'],
    ['Use paper reading theme', '使用纸张阅读主题', 'ペーパー読書テーマを使う'],
    ...
    ['Review and correct the mission before creating the course.', '创建课程前，请检查并纠正学习任务。', 'コースを作成する前に、学習ミッションを確認して修正してください。'],
    ['Correct any field, save the changes, then confirm the mission.', '修改需要调整的字段，保存后再确认学习任务。', '必要な項目を修正して保存し、その後ミッションを確定してください。'],
    ['Mission title', '学习任务标题', 'ミッション名'],
    ['Edit mission', '修改学习任务', 'ミッションを編集'],
    ['Constraints', '约束条件', '制約'],
    ['Out of scope', '暂不处理', '対象外'],
    ['One item per line', '每行一项', '1 行に 1 項目'],
    ['Save changes', '保存修改', '変更を保存'],
    ['Cancel editing', '取消修改', '編集をキャンセル'],
    ['Complete every mission field and add at least one item to each list.', '请填写所有学习任务字段，并在每个列表中至少添加一项。', 'すべてのミッション項目を入力し、各リストに少なくとも 1 項目追加してください。'],
    ['Saving mission changes…', '正在保存学习任务修改…', 'ミッションの変更を保存しています…'],
    ['Mission changes saved. Review once more, then confirm and create the course.', '学习任务已保存。请再次检查，然后确认并创建课程。', 'ミッションの変更を保存しました。もう一度確認してから、確定してコースを作成してください。'],
    ['Mission changes could not be saved.', '学习任务修改未能保存。', 'ミッションの変更を保存できませんでした。'],
    ['The learning mission is ready.', '学习任务已整理完成。', '学習ミッションの準備ができました。'],
    ['Confirm learning mission', '确认学习任务', '学習ミッションを確定'],
    ['Check and correct learning mission', '检查并纠正学习任务', '学習ミッションを確認・修正'],
    ['Learning mission needs attention', '需要处理学习任务', '学習ミッションの確認が必要です'],
    ['Learning material', '学习材料', '学習教材'],
    ['Learning goal', '学习目标', '学習目標'],
    ['Back to course library', '返回课程库', 'コースライブラリに戻る'],
    ['The learning mission needs attention before the course can be created.', '创建课程前，需要先处理学习任务。', 'コースを作成する前に、学習ミッションを確認してください。'],
    ['The learning mission was not completed', '学习任务没有整理完成', '学習ミッションの準備が完了しませんでした'],
    ['Your material and answers are preserved. You can retry.', '材料和已经填写的内容都已保留，可以重试。', '教材と入力済みの回答は保存されています。再試行できます。'],
    ['Retry continues from the current interview. You do not need to upload the material again, and course generation will not start early.', '重试会从当前访谈继续，无需重新上传材料，也不会提前开始生成课程。', '再試行は現在のインタビューから続きます。教材の再アップロードは不要で、コース生成が先に始まることもありません。'],
    ['Continue preparing', '继续整理', '準備を続ける'],
    ['Choose the closest situation. Lucubro uses it to shape examples and practice.', '选择最接近的使用场景。Lucubro 会据此安排案例和练习。', '最も近い利用場面を選んでください。Lucubro が例と練習の構成に反映します。'],
    ['Additional details (optional)', '补充说明（可选）', '補足説明（任意）'],
    ['Add your specific situation, goal, or constraints. You can also continue without extra details.', '可以补充你的具体场景、目标或限制；不填也可以继续。', '具体的な状況、目標、制約を補足できます。追加情報なしでも続行できます。'],
    ['Describe your specific goal or use case.', '写下你的具体目标或使用场景。', '具体的な目標や利用場面を書いてください。'],
    ...
    ['Explore a sample course', '查看示例课程', 'サンプルコースを見る'],
    ['Sample journey', '示例旅程', 'サンプルの流れ'],
    ['See one finished learning workspace', '查看一个完整的学习工作区', '完成済みの学習ワークスペースを見る'],
    ['Open an existing sample course to inspect the lesson, practice, source, notes, and assistant before creating your own.', '先打开现有示例课程，查看课节、练习、原文、笔记和助手，再创建自己的课程。', '既存のサンプルコースを開き、レッスン、練習、原文、ノート、アシスタントを確認してから自分のコースを作成します。'],
    ['Open sample lesson', '打开示例课节', 'サンプルレッスンを開く'],
    ['Create a course from your material', '用自己的材料创建课程', '自分の教材からコースを作成'],
    ['Current objective', '当前目标', '現在の目標'],
    ['Continue from the current lesson', '从当前课节继续', '現在のレッスンから続ける'],
    ['Latest evidence', '最近证据', '最新の学習証拠'],
    ['No practice or notes yet', '还没有练习或笔记', '練習やノートはまだありません'],
    ['Resume point', '继续位置', '再開位置'],
    ['Waiting for the first lesson', '等待第一课', '最初のレッスンを待っています'],
    ['Continue current lesson', '继续当前课节', '現在のレッスンを続ける'],
    ['Practice completed in this lesson', '已完成本课练习', 'このレッスンの練習を完了しました'],
    ['Note saved in this lesson', '已保存本课笔记', 'このレッスンにノートを保存しました'],
    ['Practice needs another attempt', '这道练习还需要再试一次', 'この練習はもう一度取り組む必要があります'],
    ['Read-only experiment', '只读实验', '読み取り専用の実験'],
    ['Course plan preview', '课程计划预览', 'コース計画プレビュー'],
    ['Check the learning path before full generation', '在完整生成前检查学习路径', '完全生成の前に学習経路を確認'],
    ['This preview uses an existing source digest and confirmed Teach Mission to expose lesson structure, source coverage, and planned practice.', '此预览使用现有材料摘要和已确认的 Teach Mission，展示课节结构、材料覆盖与计划练习。', 'このプレビューは既存の教材要約と確認済みの Teach Mission を使い、レッスン構成、教材の範囲、予定する練習を示します。'],
    ['This preview does not start generation or save changes.', '此预览不会开始生成，也不会保存更改。', 'このプレビューは生成を開始せず、変更も保存しません。'],
    ['Plan inputs', '计划输入', '計画の入力'],
    ['Source understanding', '材料理解', '教材の理解'],
    ['Teach Mission', 'Teach Mission', 'Teach Mission'],
    ['Planned lessons', '计划课节', '予定するレッスン'],
    ['What the course will ask you to do', '课程将要求你完成什么', 'コースで何を行うか'],
    ['Objective', '目标', '目標'],
    ['Source coverage', '材料覆盖', '教材の範囲'],
    ['Planned practice', '计划练习', '予定する練習'],
    ['A source about how constraints shape choices, examples, and practical action.', '一份讨论限制如何影响选择、案例和实际行动的材料。', '制約が選択、事例、実践的な行動をどう形づくるかを扱う教材です。'],
    ['Explain the central idea, judge where it applies, and complete one real-world transfer task.', '解释核心观点，判断它适用于哪里，并完成一个现实迁移任务。', '中心となる考え方を説明し、適用できる場面を判断して、現実への転移課題を一つ完了します。'],
    ['Recognize the useful constraint', '识别有用的限制', '役に立つ制約を見分ける'],
    ['Distinguish a productive boundary from a simple obstacle.', '区分能促进行动的边界与单纯障碍。', '行動を生む境界と単なる障害を区別します。'],
    ['Core argument and opening example', '核心论点与开篇案例', '中心となる主張と導入事例'],
    ['Classify two cases and explain the difference.', '对两个案例分类并解释差异。', '二つの事例を分類し、違いを説明します。'],
    ['Test the idea against a real case', '用真实案例检验观点', '現実の事例で考え方を確かめる'],
    ['Use evidence from the source to judge a workplace scenario.', '使用材料证据判断一个工作情境。', '教材の根拠を使って職場の状況を判断します。'],
    ['Worked example and counterexample', '完整案例与反例', '具体例と反例'],
    ['Choose a response and defend it with source evidence.', '选择一种应对方式，并用材料证据说明理由。', '対応を一つ選び、教材の根拠で説明します。'],
    ['Transfer the method', '迁移这个方法', '方法を転移する'],
  ...
  function routeMetadata(pathname = location.pathname) {
    ...
    if (pathname === '/artifact/new') {
      return {
        title: 'New artifact | Lucubro',
        description: 'Create a local artifact task and optionally link course material.',
      };
    }
    if (pathname.startsWith('/artifact/')) {
      return {
        title: 'Artifact | Lucubro',
        description: 'Review an artifact task, its standards, linked course, and current gaps.',
      };
    }
