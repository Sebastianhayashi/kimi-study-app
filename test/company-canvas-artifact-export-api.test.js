...
async function withServer(options, prepare, run) {
  ...
  try {
    return await run(`http://127.0.0.1:${address.port}`, instance);
  } finally {
...
function seed(instance) {
  ...
  instance.canvasArtifactStore.create({
    id: 'artifact_export_api',
    workId: 'work_export_api',
    title: 'Export Fixture',
    blocks: [
      {
        id: 'block_export_api_claim',
        type: 'claim',
        material: true,
      ...
      {
        id: 'block_export_api_interaction',
        type: 'interaction',
...
test('Work Artifact endpoint advertises export capability and serves Markdown/PDF from canonical IR', async (t) => {
  ...
  await withServer({
  ...
  }, async (instance) => seed(instance), async (baseUrl) => {
    ...
    const markdownResponse = await fetch(`${baseUrl}/api/company/works/work_export_api/artifacts/artifact_export_api/export.md`);
    assert.equal(markdownResponse.status, 200);
    assert.match(markdownResponse.headers.get('content-type'), /^text\/markdown/);
    assert.match(markdownResponse.headers.get('content-disposition'), /artifact_export_api\.md/);
    ...
    const pdfResponse = await fetch(`${baseUrl}/api/company/works/work_export_api/artifacts/artifact_export_api/export.pdf`);
    assert.equal(pdfResponse.status, 200);
    assert.equal(pdfResponse.headers.get('content-type'), 'application/pdf');
    assert.match(pdfResponse.headers.get('content-disposition'), /artifact_export_api\.pdf/);
    const pdf = Buffer.from(await pdfResponse.arrayBuffer());
    assert.equal(pdf.subarray(0, 5).toString('ascii'), '%PDF-');
    assert.equal(pdfDocuments.length, 1);
    assert.equal(pdfDocuments[0].artifactId, 'artifact_export_api');
...
test('Artifact export stays Work-scoped and PDF fails closed when its engine is unavailable', async (t) => {
  ...
  await withServer({
    ...
    const wrongWork = await fetch(`${baseUrl}/api/company/works/work_other_export/artifacts/artifact_export_api/export.md`);
