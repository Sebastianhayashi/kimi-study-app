...
function validateLessonSpec(spec) {
  ...
  for (const [index, activity] of (spec.activities || []).entries()) {
    ...
    if (activity.type === 'ordering') {
      const itemIds = new Set((activity.items || []).map((item) => item && item.id));
...
function selectMisconception(activity, response) {
  ...
  const option = (activity.options || []).find((item) => item.id === response);
  if (!option || !option.misconceptionId) return null;
  return (activity.misconceptions || []).find((item) => item.id === option.misconceptionId) || null;
}
...
function scoreActivity(activity, response) {
  ...
  if (activity.type === 'single-choice') {
    ...
    passed = (activity.acceptedAnswers || []).some((answer) => normalizeText(answer, settings) === actual);
    correct = passed;
  } else if (activity.type === 'ordering') {
    const actual = Array.isArray(response) ? response.map(String) : [];
    const expected = (activity.correctOrder || []).map(String);
    passed = actual.length === expected.length && actual.every((value, index) => value === expected[index]);
    correct = passed;
...
function computeClaimProgress(spec, attempts) {
  ...
  for (const claim of spec.claims || []) {
    const activities = (spec.activities || []).filter((activity) => activity.claimId === claim.id && activity.stage !== 'worked-example');
