...
function deriveNextLessonStatus(job, runtime) {
  const lessons = Number(runtime.lessons || 0);
  const assessments = Number(runtime.assessments || 0);
  const baselineLessons = Number(job.baselineLessons || 0);
  const baselineAssessments = Number(job.baselineAssessments || 0);
  const failed = job.stage === 'failed';
  const lessonWritten = lessons > baselineLessons;
  const assessmentWritten = assessments > baselineAssessments;
  const validating = job.phase === 'validating';
  const ready = job.stage === 'ready' && lessonWritten && assessmentWritten;

  let progress = 8;
  let canvasVariant = 'material';
  if (job.phase) progress = 20;
  if (lessonWritten) {
    progress = 65;
    canvasVariant = 'assembly';
  }
  if (assessmentWritten) {
    progress = 85;
  ...
  const ordered = [
    ['context', '读取学习记录与下一学习目标', Boolean(job.phase) || lessonWritten],
    ['lesson', '写入下一课页面', lessonWritten],
    ['assessment', '写入互动练习规格', assessmentWritten],
    ['validate', '检查新增课程文件', ready],
  ];
  ...
  let currentMessage = job.currentMessage || '正在读取学习记录并确定下一学习目标…';
  if (lessonWritten && !assessmentWritten) currentMessage = '课节页面已经写入，正在生成互动练习…';
  if (assessmentWritten && !validating) currentMessage = '互动练习已经写入，正在准备验证…';
  if (validating) currentMessage = '正在检查新增课节、活动挂载和评分规格…';
  if (ready) currentMessage = '下一课已准备好';
  if (failed) currentMessage = '下一课生成没有完成，请重试';

  return {
    progress,
    phase: PHASE_BY_VARIANT[canvasVariant] || 'extracting',
    currentMessage,
    canvasVariant,
    history,
    preview: {
      ...
      accepted: null,
      rejected: null,
      lessonNumber: baselineLessons + 1,
    },
  };
}

function deriveGenerationStatus(courseDir, job = {}, runtime = {}) {
  if (job.kind === 'next-lesson') return deriveNextLessonStatus(job, runtime);
