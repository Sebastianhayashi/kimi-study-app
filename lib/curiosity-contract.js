...
function displayText(value, limit) {
  if (value && typeof value === 'object') {
    const candidate = value.label ?? value.text ?? value.title ?? value.name ?? value.ref ?? value.href ?? value.id;
    return text(candidate, limit);
  }
...
function normalizeCuriosityCard(raw, index = 0) {
  ...
  const options = Array.isArray(prediction.options)
    ? prediction.options.map((item) => displayText(item, 180)).filter(Boolean).slice(0, 4)
    : [];
  ...
  return {
    ...
    source: {
      label: displayText(source.source?.label || source.sourceLabel, 240),
      href: /^https?:\/\//i.test(String(source.source?.href || '')) ? String(source.source.href).slice(0, 1200) : '',
      refs: [source.source?.refs, source.sourceRefs]
        .find(Array.isArray)
        ?.map((item) => displayText(item, 300))
        .filter(Boolean)
        .slice(0, 8) || [],
    },
    scores: {
