...
function sessionIdFromStderr(stderr, fallback = null) {
  const match = String(stderr || '').match(/(?:kimi\s+-r|--resume)\s+(session_[a-z0-9_-]+)/i);
  return match ? match[1] : fallback;
}

function sessionIdFromStreamMessage(message, fallback = null) {
  if (!message || message.role !== 'meta' || message.type !== 'session.resume_hint') return fallback;
  const sessionId = typeof message.session_id === 'string' ? message.session_id.trim() : '';
  return sessionId || fallback;
}

function sessionArgs(args, { cont, sessionId }) {
...
function runWire({ cwd, prompt, cont, sessionId = null, model, skillsDir, onEvent = () => {}, spawnImpl = spawn, initializeTimeoutMs = 12000 }) {
  return new Promise((resolve, reject) => {
    const args = ['--wire', '--auto', '-m', model, '--skills-dir', skillsDir];
    sessionArgs(args, { cont, sessionId });
    ...
    function handle(message) {
      ...
      if (message.id === promptId) {
        ...
        finish(null, {
          ...
          mode: 'wire',
          sessionId: sessionIdFromStderr(stderr, sessionId),
        });
        return;
      }
...
function runStreamJson({ cwd, prompt, cont, sessionId = null, model, skillsDir, onEvent = () => {}, spawnImpl = spawn }) {
  return new Promise((resolve, reject) => {
    ...
    const parser = createLineParser((line) => {
      ...
      structured = true;
      const parsedSessionId = sessionIdFromStreamMessage(message);
      if (parsedSessionId) {
        structuredSessionId = parsedSessionId;
        return;
      }
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        resolve({
          ...
          status: 'finished',
          mode: 'stream-json',
          sessionId: structuredSessionId || sessionIdFromStderr(stderr, sessionId),
        });
      }
      else {
...
function runPlain({ cwd, prompt, cont, sessionId = null, model, skillsDir, onEvent = () => {}, spawnImpl = spawn }) {
  return new Promise((resolve, reject) => {
    const args = ['-m', model, '--skills-dir', skillsDir];
    sessionArgs(args, { cont, sessionId });
    args.push('-p', prompt);
    ...
    child.on('close', (code) => {
      if (code === 0) {
        resolve({
          ...
          status: 'finished',
          mode: 'text',
          sessionId: sessionIdFromStderr(err, sessionId),
        });
      }
...
async function runTrackedKimi(options) {
  const runners = new Map([
    ['wire', runWire],
    ['stream-json', runStreamJson],
    ['text', runPlain],
  ]);
  const preferred = runners.has(options.preferredMode) ? options.preferredMode : null;
  const order = [...new Set([preferred, 'wire', 'stream-json', 'text'].filter(Boolean))];
  let lastError = null;

  for (const [index, mode] of order.entries()) {
    try {
      return await runners.get(mode)(options);
    } catch (error) {
      lastError = error;
      const finalAttempt = index === order.length - 1;
      if (finalAttempt || !error.canFallback) throw error;
    }
  }

  throw lastError || new Error('No Kimi generation mode is available');
}

module.exports = {
  PROGRESS_TOOL,
  sessionIdFromStderr,
  sessionIdFromStreamMessage,
  createLineParser,
  runWire,
  runStreamJson,
  runPlain,
