...
function safeUnitEnvironment({ privateHome, privateCodexHome, runtimePath }) {
  return [
    ...
    `--setenv=CODEX_HOME=${privateCodexHome}`,
...
function createSystemdCodexAuthorityBoundary({
  ...
  const authorityStateRoot = path.resolve(text(stateRoot) || '');
  if (!text(stateRoot) || !path.isAbsolute(stateRoot)) throw new Error('Codex authority stateRoot must be an absolute path.');
  ...
  function spawnCodex({ command, args, options = {}, policy, attestation } = {}) {
    ...
    const systemdArgs = [
      ...
      `--property=BindReadOnlyPaths=${installRoot}`,
      `--property=BindPaths=${runStateRoot}`,
      policy.workspaceWrite
        ? `--property=BindPaths=${cwd}`
        : `--property=BindReadOnlyPaths=${cwd}`,
      ...(policy.networkAccess ? [] : ['--property=PrivateNetwork=yes']),
      ...safeUnitEnvironment({ privateHome, privateCodexHome, runtimePath: unitPath }),
      codexBin,
      'app-server',
    ];
