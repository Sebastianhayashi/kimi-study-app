...
function buildSystemdSandboxArgs({
  ...
  let runtimeLimit = null;
  if (runtimeMaxSec != null) {
    runtimeLimit = Number(runtimeMaxSec);
    if (!Number.isSafeInteger(runtimeLimit) || runtimeLimit <= 0) {
      throw new Error('Systemd sandbox runtimeMaxSec must be a positive integer.');
    }
  }
  return [
    '--user',
    ...
    '--property=UMask=0077',
    ...(runtimeLimit == null ? [] : [`--property=RuntimeMaxSec=${runtimeLimit}s`]),
