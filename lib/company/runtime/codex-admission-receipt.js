...
function verifyCodexAdmissionReceipt(receipt, {
  ...
  const unknown = [];
  const mismatches = [];
  const mismatch = (field, expected, observed) => mismatches.push({ field, expected, observed: observed == null ? observed : clone(observed) });
  ...
  const catalog = receipt.catalog && typeof receipt.catalog === 'object' ? receipt.catalog : {};
  const catalogModelId = text(catalog.modelId);
  requireField(Boolean(catalogModelId), 'catalog.modelId');
  if (catalog.displayName !== 'Luna Max') mismatch('catalog.displayName', 'Luna Max', catalog.displayName);
  if (catalog.uniqueMatch !== true) mismatch('catalog.uniqueMatch', true, catalog.uniqueMatch);

  let profileResult = null;
  if (catalogModelId) {
    const approvedProfile = createApprovedCodexProfile({ modelId: catalogModelId });
    profileResult = verifyCodexProfile({ policy: approvedProfile, observed: receipt.profile });
    ...
    for (const field of ['modelId', 'mode', 'fast', 'permissionProfile']) unknown.push(`profile.${field}`);
  }
  ...
  if (threadModelId && catalogModelId && threadModelId !== catalogModelId) mismatch('thread.modelId', catalogModelId, threadModelId);
  ...
  else if (thread.serviceTier !== null) mismatch('thread.serviceTier', null, thread.serviceTier);
  ...
  requireField(Boolean(text(authority.boundaryId)), 'authority.boundaryId');
  ...
  return {
    ...
    profileName: 'Luna Max',
    modelId: catalogModelId || null,
    unknown,
    mismatches,
    bundleDigests,
    authority: text(authority.boundaryId)
      ? { boundaryId: authority.boundaryId, enforced: authority.enforced === true, probes: clone(probes) }
      : null,
