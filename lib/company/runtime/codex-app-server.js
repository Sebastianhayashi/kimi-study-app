...
function admittedProfile(admission) {
  ...
  if (admission.admitted !== true) {
    throw new Error(admission.reason || 'Verified gpt-5.6-luna max-effort admission is required for real Codex execution.');
  }
  const verified = verifyCodexProfile({
    policy: createApprovedCodexProfile(),
    observed: admission,
  });
  if (!verified.admitted) {
    throw new Error('Verified Codex execution profile must be gpt-5.6-luna / max effort / default mode / Fast off / full-access.');
  }
  if (!nonEmptyString(admission.providerPermissionProfileId)) {
    throw new Error('Verified Codex admission is missing provider permission profile id.');
  }
  return {
    ...
    mode: admission.mode,
    fast: admission.fast,
    permissionProfile: admission.permissionProfile,
...
function assertRequestUsesAdmission(request, profile) {
  ...
  if (request.model != null && String(request.model).trim() !== profile.modelId) {
    throw new Error(`Codex Run must use admitted model ${profile.modelId}.`);
  }
...
function assertThreadMatchesAdmission(threadResult, profile) {
  ...
  if (observedModel !== profile.modelId) {
    throw new Error(`Codex thread model mismatch: expected ${profile.modelId}, observed ${String(observedModel || 'unknown')}.`);
  }
  ...
  return {
    ...
    async available() {
      if (admission && admission.admitted !== true) {
        return { available: false, mode: 'app-server', reason: admission.reason || 'Verified gpt-5.6-luna max-effort admission is required.' };
      }
