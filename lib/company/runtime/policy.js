...
const CODEX_ADMISSION_REQUIRED_REASON = 'Real Codex is blocked until gpt-5.6-luna / max effort / default mode / Fast off / full-access admission is verified.';
const LUNA_ONLY_REASON = 'Only Codex gpt-5.6-luna with max effort, default mode, Fast off, and full access is permitted for Lucubro AI execution.';
...
function approvedAdmission(admission) {
  return Boolean(
    admission
...
function applyRuntimePolicy(registry, {
  ...
  for (const id of REAL_RUNTIME_IDS) {
    ...
    if (!approvedAdmission(admission)) {
