...
const { createSkillOutputIngestor } = require('./skill-output-ingestor');
...
function createRunOrchestrator({
  ...
  const outputIngestor = skillOutputIngestor || (evidenceStore
    ? createSkillOutputIngestor({ evidenceStore, approvalBroker })
    : null);
  if (outputIngestor && typeof outputIngestor.ingest !== 'function') {
    throw new Error('Run orchestrator skillOutputIngestor must expose ingest()');
  }
  const tasks = new Map();
  ...
  function persistInputEvidence(runId, input) {
    const items = Array.isArray(input.inputEvidence) ? input.inputEvidence : [];
    if (!items.length) return [];
    if (!evidenceStore) throw new Error('Run input Evidence requires EvidenceStore');
    return items.map((item) => evidenceStore.create({
      runId,
      workId: input.workId,
      workerId: input.workerId,
      kind: item.kind,
  ...
  async function execute(runId, input) {
    try {
      ...
      for await (const event of runtime.run({
        ...
        inputEvidence: Array.isArray(input.inputEvidence) ? input.inputEvidence.map((item) => ({ ...item, metadata: { ...(item.metadata || {}) } })) : [],
        skillMount,
        ...
        if (event.type === 'skill.output') {
          if (!outputIngestor) {
            runStore.appendEvent(runId, {
              ...
              reason: 'Skill output ingestion is not configured for this Run.',
              skillId: event.skillId || null,
              subrunId: input.subrunId || null,
            });
            continue;
          }
          const ingested = await outputIngestor.ingest({
            runId,
            workId: input.workId,
            workerId: input.workerId,
            subrunId: input.subrunId || null,
            skillId: event.skillId || null,
            delegationEnvelope: envelope,
            fileDeliverables: Array.isArray(input.fileDeliverables) ? input.fileDeliverables : [],
            output: event.output,
          });
          if (ingested && ingested.event) runStore.appendEvent(runId, ingested.event);
          continue;
        }
  ...
  async function start(input) {
    const id = createId();
    const delegationEnvelope = createDelegationEnvelope(input.delegationEnvelope);
    const normalizedInput = { ...input, delegationEnvelope };
    ...
    let inputEvidence;
    try {
      inputEvidence = persistInputEvidence(id, normalizedInput);
      for (const evidence of inputEvidence) {
        runStore.appendEvent(id, { type: 'evidence.received', evidence });
      }
    } catch (error) {
      runStore.appendEvent(id, { type: 'run.failed', error: error.message });
      runStore.update(id, { status: 'failed', error: error.message });
      throw error;
    }

    const task = execute(id, { ...normalizedInput, inputEvidence }).finally(() => tasks.delete(id));
    tasks.set(id, task);
    return run;
  }
