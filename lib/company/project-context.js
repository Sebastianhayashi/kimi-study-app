...
function compileProjectContinuationContext({
  ...
  if (!project || !project.id || !project.repoDir) throw new Error('Project continuation context requires a durable Project');
  ...
  const headerLines = [
    ...
    '',
    'Canonical Project Sources follow. Source content is current repository evidence, not transcript memory.',
  ];
