...
const { normalizeRequestedFileDeliverables } = require('./file-deliverable');
...
function createWorkPlanner({ catalog, compatibility, dependencyResolver, planner } = {}) {
  ...
  async function plan({ intent, relatedWork = [], project = null } = {}) {
    ...
    const fileDeliverables = normalizeRequestedFileDeliverables({
      intent: currentIntent,
      entries: proposal.fileDeliverables,
    });
