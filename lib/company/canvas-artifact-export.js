...
function markdownList(items) {
  return (Array.isArray(items) ? items : []).map((item) => `- ${String(item)}`).join('\n');
}
...
function fallbackMarkdown(fallback) {
  ...
  if (fallback.type === 'code') {
    const fence = codeFence(fallback.content.text);
    return `${fence}${fallback.content.language || ''}\n${fallback.content.text || ''}\n${fence}`;
  }
  if (fallback.content && typeof fallback.content.text === 'string') return fallback.content.text;
...
function renderCanvasArtifactExportMarkdown(document, { figurePathFor = null } = {}) {
  ...
  const parts = [`# ${requiredText(document.title, 'Canvas Artifact export title')}`];

  for (const block of document.blocks) {
    if (block.type === 'heading') parts.push(`## ${block.text}${sourceSuffix(document, block.evidenceRefs)}`);
    else if (block.type === 'paragraph') parts.push(`${block.text}${sourceSuffix(document, block.evidenceRefs)}`);
    else if (block.type === 'claim') parts.push(`**Claim.** ${block.text}${sourceSuffix(document, block.evidenceRefs)}`);
    else if (block.type === 'quote') parts.push(`> ${block.text.replace(/\r?\n/g, '\n> ')}${sourceSuffix(document, block.evidenceRefs)}`);
    else if (block.type === 'callout') parts.push(`**Note.** ${block.text}${sourceSuffix(document, block.evidenceRefs)}`);
      ...
      parts.push(`${fence}${block.language || ''}\n${block.text || ''}\n${fence}${sourceSuffix(document, block.evidenceRefs)}`);
    } else if (block.type === 'figure') {
      const embeddedPath = block.embeddingEligibility === 'embed' && figurePathFor ? figurePathFor(block) : null;
      const lines = embeddedPath
        ? [`![${block.alt}](${embeddedPath})${sourceSuffix(document, block.evidenceRefs)}`]
        : [`**Figure: ${block.alt}.**${sourceSuffix(document, block.evidenceRefs)}`];
      if (block.caption) lines.push(block.caption);
      lines.push(`Embedding eligibility: ${block.embeddingEligibility}`);
      if (block.rightsStatus) lines.push(`Rights: ${block.rightsStatus}`);
      if (block.sourcePage) lines.push(`Source page: ${block.sourcePage}`);
      parts.push(lines.join('\n'));
    } else if (block.type === 'static-interaction') {
      parts.push(`**Interactive prompt.** ${block.prompt}\n\nStatic export fallback:\n\n${fallbackMarkdown(block.fallback)}`);
    } else if (block.type === 'file-reference') {
      parts.push(`**File: ${block.label}.** ${block.path}${sourceSuffix(document, block.evidenceRefs)}`);
    }
  }
  ...
  if (document.sources.length) {
    ...
    document.sources.forEach((source, index) => {
      const lines = [`${index + 1}. **${source.label}**`];
      if (source.publisher) lines.push(`Publisher: ${source.publisher}`);
      if (source.sourcePage) lines.push(`Source: ${source.sourcePage}`);
      if (source.originalAsset) lines.push(`Original asset: ${source.originalAsset}`);
      if (source.rightsStatus) lines.push(`Rights: ${source.rightsStatus}`);
      if (source.kind === 'source-image' || String(source.mimeType || '').startsWith('image/')) {
        lines.push(`Embedding eligibility: ${source.embeddingEligibility}`);
      }
      lines.push(`Evidence: ${source.evidenceId} · sha256:${source.sha256}`);
