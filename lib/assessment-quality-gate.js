...
const OBSERVABLE_ZH_ACTIONS = [
  '解释', '判断', '诊断', '应用', '创作', '分析', '比较', '区分', '识别', '评估', '验证',
  '设计', '编写', '制定', '修订', '改写', '选择', '推断', '论证', '构建', '演示', '解决',
];
const VAGUE_ZH_ACTIONS = ['了解', '理解', '熟悉', '掌握', '知道'];
const OBSERVABLE_EN_ACTIONS = [
  'explain', 'judge', 'diagnose', 'apply', 'create', 'analyze', 'analyse', 'compare', 'distinguish',
  'identify', 'evaluate', 'verify', 'design', 'write', 'revise', 'choose', 'infer', 'argue', 'build',
  'demonstrate', 'solve',
];
const VAGUE_EN_ACTIONS = ['understand', 'know', 'learn about', 'be familiar with'];
...
function normalizedClaimAction(value) {
  return normalizedText(value)
    .replace(/^[\s:：,，.。;；!?！？\-–—]+/, '')
    .replace(/^(?:学习者|学生|你|本课学习者)\s*(?:将|应当|应该|能够|能|可以|会|需|需要)?\s*/u, '')
    .replace(/^(?:the\s+learner|learners?|students?|you)\s+(?:will|should|can|must|need\s+to)\s+/i, '')
    .replace(/^(?:be\s+able\s+to|able\s+to|can\s+|to\s+)/i, '')
    .trim();
}

function startsWithAction(text, actions) {
  return actions.some((action) => text === action
...
function vagueClaimLabelReason(label) {
  const action = normalizedClaimAction(label);
  if (!action) return null;
  if (startsWithAction(action, OBSERVABLE_ZH_ACTIONS) || startsWithAction(action, OBSERVABLE_EN_ACTIONS)) return null;
  const vagueZh = VAGUE_ZH_ACTIONS.find((item) => action === item || action.startsWith(item));
  if (vagueZh) return `claim.label must start with an observable action, not “${vagueZh}”`;
  const vagueEn = VAGUE_EN_ACTIONS.find((item) => action === item || action.startsWith(`${item} `) || action.startsWith(`${item}:`));
  if (vagueEn) return `claim.label must start with an observable action, not “${vagueEn}”`;
  return null;
}
...
function auditAssessmentQuality(rawSpec, context = {}) {
  ...
  if (isObject(claim)) {
    ...
    const vagueLabel = vagueClaimLabelReason(claim.label);
    if (vagueLabel) blockers.push(vagueLabel);
  }
