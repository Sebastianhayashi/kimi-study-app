...
function markMissionPlanning(courseDir, { retry = false, now = Date.now() } = {}) {
  ...
  return updateOnboarding(courseDir, record, {
    ...
    mission: { mode: 'standard', status: 'planning', question: null, errorCode: null, errorMessage: null },
  }, now);
}

function markMissionQuestion(courseDir, { question, materialSummary = '', now = Date.now() }) {
  const record = readOnboarding(courseDir);
