...
function inspectSourceFile(file, descriptor, now = Date.now()) {
  ...
  if (descriptor.format === 'pdf') {
    ...
    // Only a structural ZIP gate stays here: the OCF mimetype entry is not
    // guaranteed to be first or stored uncompressed in the wild, so scanning
    // the file head for the literal string rejects valid books. Deep
    // validation (mimetype entry content) runs async in validateEpubArchive.
    const zipHeader = head.length >= 4 && head[0] === 0x50 && head[1] === 0x4b
      && ((head[2] === 0x03 && head[3] === 0x04) || (head[2] === 0x05 && head[3] === 0x06));
    if (!zipHeader) {
      throw new OnboardingError('INVALID_EPUB', 'EPUB 文件结构无效', 422);
    }
  } else {
...
function normalizeCourseMode(value) {
  return value === 'goal' ? 'goal' : 'student';
}
...
function createOnboardingRecord({ courseId, source, mode = 'student', locale = 'en', now = Date.now() }) {
  ...
  return {
    ...
    updatedAt: timestamp,
    profile: {
      mode: normalizeCourseMode(mode),
      locale: normalizeCourseLocale(locale),
    },
    source: { ...source },
    inspection: {
      status: 'pending',
...
function mergeRecord(record, patch) {
  return {
    ...record,
    ...patch,
    profile: { ...(record.profile || { mode: 'student', locale: 'en' }), ...(patch.profile || {}) },
...
function createCourseDraft({ dataRoot, tempFile, originalFilename, mimeType, sizeBytes, title, mode = 'student', locale = 'en', now = Date.now() }) {
  ...
  try {
    ...
    record = createOnboardingRecord({ courseId, source, mode, locale, now });
...
function publicMissionFailure(error) {
  const code = String(error && error.code || 'MISSION_FAILED');
  const message = String(error && error.message || 'Mission 访谈没有完成');
  if (code === 'ENOENT' || /(?:spawn\s+kimi|Unable to start Kimi Wire).*ENOENT/i.test(message)) {
    return {
      code: 'KIMI_CLI_UNAVAILABLE',
      message: '未检测到 Kimi CLI。请先安装并登录 Kimi CLI，然后点击“继续整理”；材料已保留。',
    };
  }
...
function markMissionFailed(courseDir, error, now = Date.now()) {
  const record = readOnboarding(courseDir);
  const failure = publicMissionFailure(error);
  return updateOnboarding(courseDir, record, {
    state: 'awaiting_mission',
    mission: {
...
// Deep EPUB validation for upload: the OCF mimetype entry must exist and
// declare the EPUB media type, but it may sit anywhere in the archive and may
// be deflated by repackers, so only a real ZIP reader can check it.
async function validateEpubArchive(file) {
  let zip;
  try {
  ...
  if (!mimetype) {
    throw new OnboardingError('INVALID_EPUB', 'EPUB 文件缺少 mimetype 声明', 422);
  }
  const declared = (await mimetype.async('string')).trim();
  if (declared !== 'application/epub+zip') {
    throw new OnboardingError('INVALID_EPUB', 'EPUB 文件 mimetype 声明无效', 422);
  }
  return true;
}
