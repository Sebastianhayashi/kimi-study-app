...
function recentArtifactFocus(courseDir) {
  const activity = readJson(path.join(courseDir, 'learning-activity.json'), []);
  const events = Array.isArray(activity) ? activity : [];
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index];
    if (!event || event.type !== 'artifact-gap-focus') continue;
    const artifactId = clip(event.artifactId, 80);
    const gapId = clip(event.gapId, 80);
    const rubricItemId = clip(event.rubricItemId, 80);
    const gapSummary = clip(event.gapSummary, 500);
    const supportKind = clip(event.supportKind, 40);
    if (!artifactId || !gapId || !rubricItemId || !gapSummary || !supportKind) continue;
    return {
      artifactId,
      gapId,
      rubricItemId,
      gapSummary,
      sourceRefs: Array.isArray(event.sourceRefs)
        ? event.sourceRefs.map((ref) => clip(ref, 120)).filter(Boolean).slice(0, 4)
        : [],
      supportKind,
...
function buildTutorContext(courseDir, requestContext = {}) {
  ...
  const artifactFocus = recentArtifactFocus(courseDir);
  ...
  if (artifactFocus) {
    const refs = artifactFocus.sourceRefs.length ? `\n- sourceRefs: ${artifactFocus.sourceRefs.join(', ')}` : '';
    sections.push(`【当前作品缺口】\n- artifact: ${artifactFocus.artifactId} / gap: ${artifactFocus.gapId} / rubric: ${artifactFocus.rubricItemId}\n- 缺口：${artifactFocus.gapSummary}\n- 支持方式：${artifactFocus.supportKind}${refs}`);
  }
  if (feedback.length) {
