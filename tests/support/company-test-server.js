...
function describeFailure({ child, stdout, stderr, lastFetchError }) {
  const exit = child.exitCode === null && child.signalCode === null
    ? null
    : { code: child.exitCode, signal: child.signalCode };
  return JSON.stringify({ exit, lastFetchError, stdout, stderr });
}
...
async function startCompanyTestServer({
  ...
  child.stdout.on('data', (chunk) => {
    ...
    const match = stdout.match(/\[lucubro-company\]\s+(http:\/\/[^\s/]+:\d+)\/company/);
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null || child.signalCode !== null) {
      throw new Error(`company-server exited before readiness: ${describeFailure({ child, stdout, stderr, lastFetchError })}`);
    }
    if (url) {
      try {
  ...
  const instance = { child, url };
  await stopCompanyTestServer(instance).catch(() => {});
  throw new Error(`company-server did not become ready: ${describeFailure({ child, stdout, stderr, lastFetchError })}`);
}
